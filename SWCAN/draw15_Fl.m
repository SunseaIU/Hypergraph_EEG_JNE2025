...
for i = 1:15
    ...
    % main program
    [Max_acc,Bestlambda,Bestalpha,Bestcreate,BestF_U,Besttheta,BestW,Y_predict,BestIter,Bestfmbhs] = HSWCAN_Fl(X,X_l,Y_l,X_u,Y_u);

    % save result
    fprintf('Bestlambda: %.4f , Bestalpha: %.4f , Bestcreate: %.2f , the best acc: %.4f \n',Bestlambda,Bestalpha,Bestcreate,Max_acc);
    result_struct = struct('Bestlambda',Bestlambda,'Bestalpha',Bestalpha,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'Besttheta',Besttheta,'BestW',BestW,'Y_predict',Y_predict,'BestIter',BestIter,'function_value',Bestfmbhs);
    save_path = ['../result/HSWCAN_Fl_patient',num2str(patient),'_process',num2str(process_param)];
...
saveas(gcf,'../result/15subjects_Fl_process5.fig');
t2 = clock;
