...
for i = 1:15
    ...
    % main program
    [Max_acc,Bestr,Bestalpha,Bestbeta,Bestcreate,BestF_U,BestM,BestH,BestW,Y_predict,BestIter,CREATE] = DHSLP_c(X,X_l,Y_l,X_u,Y_u);
    
    % save result
    result_struct = struct('Bestr',Bestr,'Bestalpha',Bestalpha,'Bestbeta',Bestbeta,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'BestM',BestM,'BestH',BestH,'BestW',BestW,'Y_predict',Y_predict,'BestIter',BestIter,'CREATE',CREATE);
    save_path = ['../result/all channels/DHSLP_c/DHSLP_c_patient',num2str(patient),'_process',num2str(process_param)];
