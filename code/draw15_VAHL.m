...
for i = 1:15
    ...
    % main program
    [Max_acc,Bestr,Bestalpha,Bestbeta,Bestcreate,BestF_U,BestM,BestH,BestW,Y_predict,BestIter,Bestfmbhs] = VAHL(X,X_l,Y_l,X_u,Y_u);
    ...
    save_path = ['../result/linux/VAHL_create_patient',num2str(patient),'_process',num2str(process_param)];
    save(save_path,'Max_acc','result_struct')

    subplot(3,5,i);
    plot(1:length(Bestfmbhs),Bestfmbhs(1:end));
    title(['Subject' num2str(i)]);
    set(gca, 'ButtonDownFcn', @showSubplot);
end
saveas(gcf,'../result/linux/VAHL_15subjects_create_process5.fig');
t2 = clock;
