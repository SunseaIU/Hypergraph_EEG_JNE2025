...
for patient = 1:10
    ...
    for fold = 1:cv.NumTestSets
        ...
        % main program
        [Max_acc,Bestalpha,Bestbeta,Bestcreate,BestF_U,BestW,Y_predict,BestIter] = DHSL_noH(X,X_l,Y_l,X_u,Y_u);

        % save result
        fprintf('Patient %02d , Fold %d : Bestalpha: %d , Bestbeta: %.4f , Bestcreate :%.2f , the best acc: %.4f \n',patient,fold,Bestalpha,Bestbeta,Bestcreate,Max_acc);
        result_struct = struct('Bestalpha',Bestalpha,'Bestbeta',Bestbeta,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'BestW',BestW,'Y_predict',Y_predict,'BestIter',BestIter);
        save_path = ['../result/session',session,'/DHSL_noH/DHSL_noH_patient',patient_str,'-',session,'process_',num2str(process_param),'_fold',num2str(fold)];
        % figure;
...
for patient = 1:10
    ...
    accuracies3(patient) = mean(fold_accuracies);
    fprintf('Patient %02d: mean acc: %.4f\n',patient, accuracies3(patient));
end
fprintf('Overall mean acc: %.4f\n', mean(accuracies3));
