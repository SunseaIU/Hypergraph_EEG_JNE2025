...
for patient = 1:10
    ...
    for fold = 1:cv.NumTestSets
        ...
        % main program
        [Max_acc,Bestr,Bestalpha,Bestbeta,Bestcreate,BestF_U,BestM,BestH,BestW,Y_predict,BestIter] = VAHL_2(X,X_l,Y_l,X_u,Y_u);

        % save result
        fprintf('Patient %02d , Fold %d : Bestr: %d , Bestalpha: %.4f , Bestbeta: %.4f , Bestcreate :%.2f , the best acc: %.4f \n',patient,fold,Bestr,Bestalpha,Bestbeta,Bestcreate,Max_acc);
        result_struct = struct('Bestr',Bestr,'Bestalpha',Bestalpha,'Bestbeta',Bestbeta,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'BestM',BestM,'BestH',BestH,'BestW',BestW,'Y_predict',Y_predict,'BestIter',BestIter);
        save_path = ['../result/session',session,'/DHSLP/DHSLP_patient',patient_str,'-',session,'process_',num2str(process_param),'_fold',num2str(fold)];
        % figure;
        % plot(1:length(Bestfmbhs),Bestfmbhs(1:end),'b-o');
        save(save_path,'Max_acc','result_struct');
        fold_accuracies(fold) = Max_acc;
    end
    accuracies(patient) = mean(fold_accuracies);
