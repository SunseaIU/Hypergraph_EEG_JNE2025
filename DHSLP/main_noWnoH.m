...
for patient = 1:10
    ...
    for fold = 1:cv.NumTestSets
        ...
        % main program
        [Max_acc,Bestr,Bestalpha,Bestcreate,BestF_U,BestM,Y_predict,BestIter] = VAHL_2_noWnoH(X,X_l,Y_l,X_u,Y_u);

        % save result
        fprintf('Patient %02d , Fold %d : Bestr: %d , Bestalpha: %.4f , Bestcreate :%.2f , the best acc: %.4f \n',patient,fold,Bestr,Bestalpha,Bestcreate,Max_acc);
        result_struct = struct('Bestr',Bestr,'Bestalpha',Bestalpha,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'BestM',BestM,'Y_predict',Y_predict,'BestIter',BestIter);
        save_path = ['../result/session',session,'/DHSLP_noWnoH/DHSLP_noWnoH_patient',patient_str,'-',session,'process_',num2str(process_param),'_fold',num2str(fold)];
        % figure;
