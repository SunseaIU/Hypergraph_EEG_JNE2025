function [Max_acc,Bestlambda,Bestalpha,Bestcreate,BestF_U,Besttheta,BestH,BestW,Y_predict,BestIter,Bestfmbhs] = DHSLF_visual(X,X_l,Y_l,X_u,Y_u)
% =========================================================================
...
for create_index = 1:length(createnums)
    for lambda_index = 1:length(lambdanums)
        for alpha_index = 1:length(alphanums)
            ...
            S = U*H*W*De1*H'*U;
            fx11 = zeros(70,1); x11 = 61; x12 = [1:60,301:310]';
            fx11 = S(x11,x12)';
...
%                 fprintf('目标函数值： %.6f ，准确度： %.6f \n',mbhs,acc);
            ...
            fprintf('create: %.2f , alpha: %.4f , lambda: %.4f ,current acc: %.4f, the best acc: %.4f \n',create,alpha,lambda,current_acc,Max_acc);
            current_acc = 0;
        end
    end
end
