function [Max_acc,Bestlambda,Bestalpha,Bestcreate,BestF_U,Besttheta,BestW,Y_predict,BestIter] = HSWCAN_Fl(X,X_l,Y_l,X_u,Y_u)
% =========================================================================
...
%             fmbhs = [fmbhs,mbhs];
            ...
            for iter = 1:MaxIteration
                % step: Update F
                L = L + eye(n)*1e-10;
                F(l+1:n,:) = -L(l+1:n,l+1:n)\L(l+1:n,1:l)*Y_l;
%                 mbhs = trace(F'*L*F) + lambda*trace(Theta*X'*L*X*Theta) + alpha*trace(W'*W);
%                 fmbhs = [fmbhs,mbhs];
%                 fprintf('目标函数值： %.6f \n',mbhs);
...
%             fprintf('create: %.2f, lambda: %.4f , alpha: %.4f ,current acc: %.4f, the best acc: %.4f \n',create,lambda,alpha,current_acc,Max_acc);
            current_acc = 0;
        end
    end
end
