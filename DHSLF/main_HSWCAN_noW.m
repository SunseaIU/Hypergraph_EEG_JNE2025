...
for patient = 1:10
    ...
    for fold = 1:cv.NumTestSets
        ...
        % main program
        [Max_acc,Bestlambda,Bestcreate,BestF_U,Besttheta,BestH,Y_predict,BestIter] = HSWCAN_noW(X,X_l,Y_l,X_u,Y_u);
        
        % save result
        fprintf('Patient %02d , Fold %d : Bestlambda: %.4f , Bestcreate: %.2f , the best acc: %.4f \n',patient,fold,Bestlambda,Bestcreate,Max_acc);
        result_struct = struct('Bestlambda',Bestlambda,'Bestcreate',Bestcreate,'BestF_U',BestF_U,'Besttheta',Besttheta,'BestH',BestH,'Y_predict',Y_predict,'BestIter',BestIter);
        save_path = ['../result/session',session,'/DHSLF_noW/DHSLF_noW_patient',patient_str,'-',session,'process_',num2str(process_param),'_fold',num2str(fold)];
